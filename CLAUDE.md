...
Агенты **не задают уточняющих вопросов** владельцу продукта в чате. Все технические решения —
выбор подхода, инструментов, архитектуры внутри согласованных ограничений, последовательность
шагов — принимаются самостоятельно. Допущения фиксируются в описании PR или комментарии к issue,
а не выносятся на обсуждение заранее.

Исключение — работа физически не может продолжаться без ответа владельца продукта:
Acceptance Criteria противоречат друг другу и не разрешаются разумным дефолтом, либо требуются
реальные секреты/креды, которых нет в проекте. Такие случаи — редкость, а не норма.

В `/pipeline` и любых автономных режимах это правило действует без исключений.
...
- **OQ-18 — Форма Админки** (закрыт 2026-08-14): отдельный SPA, не только Swagger UI. Дизайн
  предоставлен владельцем продукта (Claude Design artifact). Решение зафиксировано в
  `docs/architecture.md`, раздел «Admin Interface». Backend для SPA реализован в M1.6
  (issues #33, #35); сам SPA — отдельный будущий issue.