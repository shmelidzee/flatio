...
### Fixed
- **PR #509 — UX-находки раздела «Чёрный список»: пустой фильтр, маркер фильтра, отмена (issues #506, #507, #508)**
  - Пустой отфильтрованный список (например «Источники» без записей) теперь показывает текст по
    типу и сохраняет строку фильтра вместо общего «список пуст» — можно переключить фильтр без
    выхода из раздела; полностью пустой список не изменил поведения
  - Активный фильтр помечается «✅ » вместо малозаметного «• »; подсказка поясняет, что номер на
    кнопке «Удалить» соответствует порядку записи в списке
  - Промпт добавления стоп-слова (и повтор при невалидном вводе) получил кнопку «Отмена» — раньше
    выйти можно было только вводом текста
- **PR #504 — Нумерация кнопок удаления в чёрном списке (issue #498)**
  - Кнопки «Удалить» показывают порядковый номер («🗑 Удалить (1)», «🗑 Удалить (2)», …) вместо
    одинакового текста на каждой строке — нумерация в рамках страницы пагинации, не сквозная
- **PR #502 — Тип недвижимости и «от собственника» в сводке подписки (issue #496)**
  - `buildCriteriaSummary()` дополнена типом недвижимости и меткой «от собственника» — оба поля
    собирались визардом, но не показывались в списке «Мои подписки», из-за чего сводка подписки,
    заданной только этими фильтрами, выглядела полностью пустой
- **PR #501 — Кнопки навигации после создания подписки (issue #493)**
  - Подтверждение успешного создания подписки несёт кнопки «🔔 Мои подписки» и «🏠 Главное меню»
    вместо голого текста без клавиатуры
- **PR #499 — Кнопка «Добавить стоп-слово» в пустом чёрном списке (issue #492)**
  - Пустой чёрный список получил кнопку «➕ Добавить стоп-слово» в клавиатуре — текст её уже
    обещал, но кнопки не было
- **PR #489 — Диагностика формата недекодируемых Kufar-фото (issue #465)**
  - WARN-лог о фото, которое `ImageIO` не смог декодировать, дополнен полем `detectedFormat`
    (WebP/AVIF/HEIC-HEIF/JPEG2000/unrecognized), определяемым по сигнатуре байт — не нужно заново
    скачивать файл по URL из лога, чтобы понять причину
  - Часть Acceptance Criteria (скачать и изучить реальные байты, оценить частоту случая)
    задокументирована как невыполнимая автономно — источник (`kufar.by`) был недоступен из
    окружения разработки на TCP-уровне

### Added
- **PR #505 — Шаг выбора города в визарде + фикс HTML-фолбэка фото Kufar (issues #503, #497)**
  - Новый шаг `CITY` в Telegram-визарде поиска/подписки — раньше `SearchFilterState.cityId`
    никогда не устанавливался из бота, хотя REST API его уже поддерживал; клавиатура строится из
    `CityService`, сводка на шаге DONE дополнена строкой «Город: …»
  - `PhotoProxyClient` посылает browser-like `User-Agent`/`Referer` при обращении к
    сконфигурированному хосту Kufar CDN и детектит HTML-ответ вместо байт изображения — часть
    фото с Kufar приходила как HTML-страница вместо картинки
- **PR #500 — Фото и ссылка на объявление в карточках избранного (issue #494)**
  - Раздел «⭐ Избранное» отправляет фото-карточку на объявление (фото или плейсхолдер, подпись,
    кнопки «Открыть объявление →» и «❌ Убрать из избранного») вместо текстового сообщения на всю
    страницу
- **PR #491 — Quiet hours для REALTIME-уведомлений (issue #411)**
  - Новый `QuietHoursEvaluator` — REALTIME-уведомления не доставляются и не учитываются в часовом
    лимите, пока подписка в окне тишины (`Europe/Minsk`); подхватываются ближайшим плановым
    прогоном дайджеста вместо отдельного таймера
  - Санитизация ISOBMFF `brand` перед WARN-логом в `SearchResultSender` (issue #490) — закрывает
    MEDIUM-находку Security Engineer с ревью PR #489 (CWE-117 log forging)

### Added
- **PR #487 — Отложенная доставка уведомлений: DIGEST и DAILY (issue #410)**
  - Уведомления доставляются не только сразу (REALTIME), но и батчами: DIGEST (по умолчанию
    каждые 3 часа, `flatio.notifications.digest.cron` / `FLATIO_NOTIFICATIONS_DIGEST_CRON`) и DAILY
    (раз в день, по умолчанию 09:00, `flatio.notifications.daily.cron` /
    `FLATIO_NOTIFICATIONS_DAILY_CRON`) — режим определяется `deliveryMode` подписки
  - `BatchNotificationSender` группирует все `PENDING`-уведомления пользователя в одно компактное
    сообщение (строка на уведомление) вместо карточки на каждое — иначе десятки уведомлений в одном
    батче зафлудили бы чат; при превышении лимита длины сообщения Telegram (4096 символов) остаток
    помечается «… и ещё N объявлений» и остаётся `PENDING` до следующего запуска
  - FR-SUB-9: REALTIME-уведомление, не доставленное из-за часового лимита пользователя
    (`flatio.notifications.realtime.max-per-hour`) дольше
    `flatio.notifications.digest.realtime-overflow-minutes` (по умолчанию 60 мин), подхватывается
    ближайшим DIGEST-запуском вместо бесконечного ожидания REALTIME-доставки
  - `TelegramChatResolver` и `NotificationStatusUpdater` вынесены из `TelegramNotificationSender`,
    чтобы REALTIME- и batch-доставка использовали одинаковый резолвинг chat ID и одинаковую запись
    метрик `flatio.notifications.sent`/`flatio.notifications.failed`
  - Новые `DigestNotificationDeliveryJob`/`DailyNotificationDeliveryJob` (`scheduler`): ошибка
    чтения или доставки батча не роняет сам scheduler; доставка отдельного уведомления внутри
    батча уже изолирована на уровне `BatchNotificationSender`
- **PR #486 — Редактирование подписки через мастер фильтров (issue #479)**
  - Кнопка «Изменить» на подписке в разделе «🔔 Мои подписки» открывает тот же мастер фильтров, что
    и /search, предзаполненный текущими критериями подписки (тип сделки, город, цена, комнаты)
  - `FILTER:SEARCH` ветвится: если сессия мастера несёт `editingSubscriptionId`, шаг завершения
    сохраняет изменения через `SubscriptionService#update` вместо запуска поиска по объявлениям
  - `SubscriptionEditState` хранит снапшот подписки на время редактирования — мастер собирает
    только критерии поиска, поэтому имя, триггеры и способ доставки подписки переносятся без
    изменений
- **PR #485 — Список подписок одним сообщением со сводкой условий поиска (issue #478)**
  - Страница подписок отправляется одним сообщением (строка на подписку: имя/статус/сводка
    критериев поиска — тип сделки, город, диапазон цены, число комнат) с одним рядом кнопок
    пауза/возобновить/удалить на подписку, вместо сообщения на каждую подписку — по образцу
    `FavoritesCallbackHandler`
  - Пагинация `SUB:PAGE:NEXT`/`SUB:PAGE:PREV`, как в разделах «Избранное» и «Чёрный список»
  - Сводка критериев строится из `SubscriptionSearchCriteria` без изменения контракта
    `SubscriptionService`; для резолва города по `cityId` в `CityService` добавлен `findById`
  - Ошибка форматирования одной подписки изолирована и не мешает отрисовке остальных подписок
    страницы
- **PR #484 — Чёрный список одним сообщением с пагинацией (issue #477)**
  - Список чёрного списка рендерится как единое сообщение на страницу (строка на запись + отдельный
    ряд кнопки удаления на запись) вместо сообщения на каждую запись — так же, как уже сделано для
    избранного
  - Пагинация `BL:PAGE:NEXT`/`BL:PAGE:PREV` поверх уже существующего фильтра по типу записи
    (LISTING/SOURCE/KEYWORD)
  - Ошибка форматирования одной записи изолирована и не ломает отрисовку остальной страницы
- **PR #483 — Избранное одним сообщением на страницу (issue #476)**
  - Список избранного рендерится как единое сообщение на страницу вместо сообщения на каждое
    объявление
  - Ошибки кнопок и форматирования по отдельным объявлениям изолированы друг от друга
- **PR #482 — Точка входа в поиск из пустых подписок (issue #475)**
  - Пустой экран «🔔 Мои подписки» вместо единственной кнопки «Главное меню» предлагает «Начать
    поиск и подписаться» — тот же вход в мастер фильтров, что и у /search, чтобы создать первую
    подписку можно было не зная заранее о существовании /search
- **PR #481 — Понятные пустые состояния Избранного и Чёрного списка (issue #474)**
  - Пустой список избранного и чёрного списка показывает подсказку и кнопку «🔍 Перейти к поиску»
    вместо тупика с одной кнопкой «Главное меню»
  - Навигация раздела «Чёрный список» теперь всегда поясняет, откуда берутся записи (карточка
    объявления в поиске / кнопка «➕ Добавить стоп-слово»)
- **PR #480 — Команды /favorites, /subscriptions, /blacklist и обновлённый /help (issue #473)**
  - Новые текстовые команды `/favorites`, `/subscriptions`, `/blacklist` открывают те же разделы,
    что и одноимённые кнопки главного меню, переиспользуя существующие callback-хендлеры без новой
    бизнес-логики
  - Команды зарегистрированы в `BotCommandsRegistrar` (видны в меню команд Telegram)
  - `/help` дополнен строками по трём новым командам
- **PR #471 — Алёрты при сбоях источников (issue #419)**
  - `SourceAlertCheckJob` (каждые 15 мин по умолчанию) проверяет два правила по каждому активному
    источнику напрямую по истории `sync_runs` (`SyncRunService`), а не через Prometheus
    Alertmanager — issue явно оставлял выбор, Alertmanager потребовал бы нового сервиса в
    `docker-compose.yml` и отдельного rules-конфига ради того, что выражается напрямую в Java
    поверх уже существующих данных
  - Правило 1: 0 успешных синков за `flatio.alerts.no-success-hours` часов (по умолчанию 3);
    источник без единого запуска (только что добавлен) исключён, не алёртит преждевременно
  - Правило 2: доля неудачных синков среди последних `flatio.alerts.error-rate-window-runs`
    (по умолчанию 5) превышает `flatio.alerts.error-rate-threshold` (по умолчанию 0.5)
  - Доставка в Telegram (`flatio.alerts.telegram.chat-id` / `FLATIO_ALERTS_TELEGRAM_CHAT_ID`) —
    без обязательного дефолта, окружение без алёртинга не падает при старте
  - Дедупликация: повторное уведомление по уже активному алёрту — не раньше
    `flatio.alerts.cooldown-hours` (по умолчанию 6) с прошлого; восстановление источника —
    отдельное уведомление (`V64__create_source_alert_state.sql`)
- **PR #470 — Управление избранным/подписками/чёрным списком в Telegram-боте (issues #457, #458, #459)**
  - Три персональных раздела (#456) из read-only списков стали полноценными экранами управления,
    поверх уже существующих `FavoriteService`/`SubscriptionService`/`BlacklistService` — REST API
    не менялся
  - ⭐ Избранное: пагинированный список (по одному сообщению на объявление + навигационное
    сообщение), кнопка «⭐ В избранное» на карточке поисковой выдачи, «❌ Убрать» в списке, пометки
    неактуально/цена изменилась
  - 🔔 Подписки: «🔔 Подписаться на этот поиск» в навигационном сообщении после поиска (создаёт
    подписку из текущего `SearchFilterState`, имя запрашивается свободным текстом), список со
    статусом/способом доставки, пауза/возобновление/удаление
  - 🚫 Чёрный список: «🚫 Скрыть объявление»/«🚫 Скрыть источник» на карточке поиска, список с
    фильтром по типу (LISTING/SOURCE/KEYWORD), добавление стоп-слова свободным текстом
    (валидация: не пусто, ≤100 символов), удаление
  - По всем трём issue AC описывал «toggle»-кнопку на карточке поиска; реализовано как раздельные
    действия (добавление с карточки / удаление из списка) — карточка не может знать состояние
    избранного пользователя без лишнего запроса к БД на каждый рендер выдачи (N+1 при пагинации),
    решение задокументировано в PR
...
- **PR #362 — Город Kufar, 404-маршрут admin SPA, обработка 429/ошибок в админке (issues #358, #359, #360)**
  - **#358** — `KufarApiClient` не заполнял поле `city` у объявлений всех шести Kufar-источников;
    ...
    того, какой источник адреса выиграл (`account_parameters`, detail-страница или `region`/`area`
    fallback). Backfill существующих строк не требует отдельной миграции — `city` обновляется у
    уже сохранённых объявлений при каждом re-ingest, а каждый Kufar full-sync job перечитывает все
    объявления категории ежедневно
  - **#359** — в admin SPA не было catch-all маршрута; несуществующий путь рендерил полностью
    пустой экран. Добавлен `path="*"` внутри `ProtectedRoute > AdminLayout` — авторизованный
    пользователь видит `NotFoundPage` с сайдбаром и ссылкой на дашборд, неавторизованный
    по-прежнему уходит на `/login`
  - **#360** — при `429` от `/api/v1/admin/**` UI молча показывал «не найдено» вместо ошибки.
    Добавлен `ApiError` (несёт HTTP-статус) и компонент `QueryErrorMessage` с отдельным
    сообщением для 429 и кнопкой «Повторить» на `ListingsPage`, `SourcesPage`, `UsersPage`;
    `QueryClient` больше не ретраит 4xx-ответы автоматически. Также заведён отдельный лимитер
    `api-admin` (240 req/60с) для `/api/v1/admin/**` вместо общего `api-authenticated`
    (60 req/60с) — доверенный внутренний трафик небольшого числа админов, не публичный API
