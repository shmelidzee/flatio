...
import com.flatio.integration.realt.config.RealtHouseRentProperties;
...
/**
 * Connector for fetching house/cottage rental listings from the realt.by website.
 *
 * <p>Fetches SSR HTML from realt.by and delegates listing extraction to {@link RealtHtmlParser}.
 * All extracted listings have {@code dealType = RENT} and {@code propertyType = HOUSE}.
 *
 * <p>Shares the {@code connector-realt} Resilience4j rate limiter with {@link RealtConnector}
 * and {@link RealtRoomConnector} — all target the same origin server (realt.by). The circuit
 * breaker is dedicated to this connector's category ({@code connector-realt-house-rent}) so a
 * run of failures here cannot open the breaker for other rental categories (issue #373).
 */
@Service
@Slf4j
public class RealtHouseRentConnector implements ListingConnector {
  ...
  private final RealtHouseRentProperties properties;
  private final RealtHtmlParser htmlParser;
  private final RealtPageContext pageContext;

  public RealtHouseRentConnector(@Qualifier("realtRestClient") RestClient restClient,
      RealtHouseRentProperties properties,
    ...
    this.pageContext = new RealtPageContext(
        ...
        DEAL_TYPE_RENT, PROPERTY_TYPE_HOUSE, FALLBACK_TITLE
    );
  }
  ...
  /**
   * Fetches all current house rental listings from realt.by by paginating through all pages.
   *
  ...
  @CircuitBreaker(name = "connector-realt-house-rent")
  ...
  /**
   * Fetches house rental listings published at or after the given timestamp (delta sync).
   *
  ...
  List<RawListing> fetchFallback(Exception e) {
    log.error("All retry attempts exhausted for RealtHouseRent fetch: source={}", properties.sourceId(), e);
    return List.of();
  }

  // Package-private: Resilience4j AOP proxy requires fallback to be accessible from the same package.
  List<RawListing> fetchDeltaFallback(Instant since, Exception e) {
    log.error("All retry attempts exhausted for RealtHouseRent delta fetch: source={}, since={}", properties.sourceId(), since, e);
    return List.of();
  }
