...
/**
 * Periodically fetches only new Kufar room sale listings since the last successful sync.
 *
 * <p>Cron schedule is configurable via {@code flatio.sync.kufar-room-sale.delta.cron}
 * (env: {@code FLATIO_SYNC_KUFAR_ROOM_SALE_DELTA_CRON}); default is every 15 minutes.
 */
@Component
@Slf4j
@RequiredArgsConstructor
public class KufarRoomSaleDeltaSyncJob {
  ...
  public void runDeltaSync() {
    ...
    try {
      ...
      if (!source.isActive()) {
        log.debug("KufarRoomSale delta sync skipped: source disabled: source={}", connector.getSourceId());
        return;
      }
      ...
      log.warn("KufarRoomSale delta sync skipped: circuit breaker OPEN");
    } catch (DataAccessException e) {
      log.warn("KufarRoomSale delta sync: DB unavailable: source={}, error={}", connector.getSourceId(), e.getMessage());
    } catch (Exception e) {
      log.error("KufarRoomSale delta sync failed: source={}, error={}", connector.getSourceId(), e.getMessage(), e);
  ...
  private void performDeltaSync(Source source, Instant since, Instant runStart) {
    log.info("KufarRoomSale delta sync started: since={}", since);
    List<RawListing> rawListings = connector.fetchDelta(since);
    if (rawListings.isEmpty()) {
      log.info("KufarRoomSale delta sync: no new listings since={}", since);
    ...
    log.info("KufarRoomSale delta sync completed: fetched={}, added={}, updated={}, errors={}, durationMs={}",
  ...
  private void performFullSyncFallback(Source source, Instant runStart) {
    ...
    if (rawListings.isEmpty()) {
      log.warn("KufarRoomSale full-sync fallback: empty list — skipping");
      return;
    }
    BatchIngestResult result = listingIngestionService.ingestBatch(rawListings, source);
    ...
    log.info("KufarRoomSale full-sync fallback completed: fetched={}, added={}, updated={}, errors={}, durationMs={}",
        rawListings.size(), result.added(), result.updated(), result.errors(), Duration.between(runStart, finish).toMillis());
  }
}
