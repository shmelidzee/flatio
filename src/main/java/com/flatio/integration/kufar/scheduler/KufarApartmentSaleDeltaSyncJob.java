...
/**
 * Periodically fetches only new Kufar apartment sale listings since the last successful sync.
 *
 * <p>Cron schedule is configurable via {@code flatio.sync.kufar-apartment-sale.delta.cron}
 * (env: {@code FLATIO_SYNC_KUFAR_APARTMENT_SALE_DELTA_CRON}); default is every 15 minutes.
 */
@Component
@Slf4j
@RequiredArgsConstructor
public class KufarApartmentSaleDeltaSyncJob {
  ...
  private final KufarApartmentSaleFullSyncJob fullSyncJob;
  ...
  @Scheduled(cron = "${flatio.sync.kufar-apartment-sale.delta.cron}")
  public void runDeltaSync() {
    Instant runStart = Instant.now();
    try {
      Source source = sourceService.findByCodeOrThrow(connector.getSourceId());
      if (!source.isActive()) {
        log.debug("KufarApartmentSale delta sync skipped: source disabled: source={}", connector.getSourceId());
        return;
      }
      ...
      if (lastRunAt.isPresent()) {
        ...
        log.info("KufarApartmentSale delta sync: FullSyncJob is already running, skipping: source={}", connector.getSourceId());
      } else {
        log.info("KufarApartmentSale: no prior successful run — falling back to full sync");
        performFullSyncFallback(source, runStart);
      }
    } catch (CallNotPermittedException e) {
      log.warn("KufarApartmentSale delta sync skipped: circuit breaker OPEN");
    } catch (DataAccessException e) {
      log.warn("KufarApartmentSale delta sync: DB unavailable: source={}, error={}", connector.getSourceId(), e.getMessage());
    } catch (Exception e) {
      log.error("KufarApartmentSale delta sync failed: source={}, error={}", connector.getSourceId(), e.getMessage(), e);
  ...
  private void performDeltaSync(Source source, Instant since, Instant runStart) {
    log.info("KufarApartmentSale delta sync started: since={}", since);
    List<RawListing> rawListings = connector.fetchDelta(since);
    if (rawListings.isEmpty()) {
      log.info("KufarApartmentSale delta sync: no new listings since={}", since);
    ...
    log.info("KufarApartmentSale delta sync completed: fetched={}, added={}, updated={}, errors={}, durationMs={}",
  ...
  private void performFullSyncFallback(Source source, Instant runStart) {
    ...
    if (rawListings.isEmpty()) {
      log.warn("KufarApartmentSale full-sync fallback: empty list — skipping");
      return;
    }
    BatchIngestResult result = listingIngestionService.ingestBatch(rawListings, source);
    ...
    log.info("KufarApartmentSale full-sync fallback completed: fetched={}, added={}, updated={}, errors={}, durationMs={}",
        rawListings.size(), result.added(), result.updated(), result.errors(), Duration.between(runStart, finish).toMillis());
  }
}
