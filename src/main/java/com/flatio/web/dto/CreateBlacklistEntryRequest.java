 ...
 * {@code LISTING}, a valid numeric source ID for {@code SOURCE}, or a non-blank stop-word (at most
 * 100 characters) for {@code KEYWORD}. This type-dependent format is validated in the service
 * layer, since it cannot be expressed with per-field bean validation constraints alone.
 */
...
public record CreateBlacklistEntryRequest(
    ...
    @Schema(description = "Listing ID (LISTING), source ID (SOURCE), or stop-word (KEYWORD)",
